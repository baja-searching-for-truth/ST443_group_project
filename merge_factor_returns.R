...
library(dplyr)
library(stringr)
#Merge factor returns from separated csv
setwd("C:\\Users\\technoplanet\\Desktop\\LSE Projects\\ST443\\Group Project\\Factor_data")
files=dir()
file_read=read.csv(files[1])
data=file_read[c('Date','fdate')]
# Sorted portfolio return, which is top 20% and last 20%. 
# AR_1 minus AR5 is the return of high minus low wrt this factor
criteria=c('AR_1','AR_5')
name_now=c("")
#A function to transfer percentage representation of number to float
transfer_percentage_to_number <- function(x)
{
  return (as.numeric(sub("%","",x))/100)
}
# get each csv and process it
for (file in files)
{
...
{
  # These two function are used to transfer year month expression
  # like 197403 to the end of this month like 1974-03-31
  # which is vital to merge our data
  add_zero <- function(a)
  {
    if (a<10)
    {
...
data=apply(data,2,as.character) %>% data.frame
write.csv(data,"factor_returns.csv")
